...
export default function Cart() {
  ...
  return (
    <>
      <Head>
        <title>Cart - Ohreassa Technology</title>
      ...
      <section className="w-full px-5 lg:px-20 my-5">
        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="lg:w-1/2 w-full flex md:justify-end mt-10 md:mt-0">
            <Link href={"/shop/"}>
              <button className="bg-mainPurple py-3 px-5 text-center font-semibold text-white text-[0.85rem] rounded-full">
                Continue Shopping
              </button>
            </Link>
          </div>
        ...
        {items.map((item: any, index: any) => (
          <motion.section
            ...
            key={item._id}
            className={`w-full ${
              index % 2 === 0 ? "bg-mainPurple" : "bg-mainYellow"
        ...
        {items.length > 0 && (
          <section className="w-full mt-20">
            <div className="w-full flex justify-between items-center mb-5">
              ...
              <p className="font-bold text-mainPurple text-[1.2rem]">
                {items.length}
              </p>
            </div>
            ...
            <Link href={`/shop/checkout`}>
              <button className=" bg-mainPurple text-center py-3 w-full text-white font-semibold rounded-lg">
                Proceed To Checkout
              </button>
            </Link>
          </section>
